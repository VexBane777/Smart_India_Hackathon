...
NOT independently verified end-to-end in the environment this module was
written in: ffmpeg was not installed there (no admin rights, network
blocked direct downloads) and could not be installed. The command
construction and pad/trim logic are unit-tested with a mocked
`subprocess.run`; the real subprocess-to-ffmpeg path is exercised only by
tests gated on `ffmpeg` being present (see tests/telechannel/test_codec.py).
A human with a real ffmpeg binary (including the AMR/GSM/Opus encoders)
must run those tests before this stage is trusted for those codecs.
...
CODEC_SPECS = {
    ...
    "libvo_amrwbenc": {"encoder": "libvo_amrwbenc", "ext": "awb", "sr": 16000, "fixed_rate": False},
