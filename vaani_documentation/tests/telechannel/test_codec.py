...
2. Tests that genuinely invoke ffmpeg end-to-end. These are gated with
   ...
   on PATH. See the module docstring in codec.py: ffmpeg was not available
   in the environment this file was authored in, so these tests have not
   been run against a real encoder. A human must install ffmpeg (with the
   AMR-NB/AMR-WB/GSM/Opus encoders as needed) and re-run this file before
   the codec round-trip stage is trusted end-to-end.
"""
